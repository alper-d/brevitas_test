...
from imports import (TrainingEpochMeters, SqrHingeLoss, weight_decay,accuracy, lr, prune_brevitas_model, log_freq, EvalEpochMeters, eval_model)
...
prune_brevitas_model(model, conv_feature_index=8, pruning_amount=pruning_amount)
prune_brevitas_model(model, conv_feature_index=11, pruning_amount=pruning_amount)
prune_brevitas_model(model, conv_feature_index=15, pruning_amount=pruning_amount)
prune_brevitas_model(model, conv_feature_index=18, pruning_amount=pruning_amount)
...
for epoch in range(starting_epoch, epochs):
    ...
    # Perform eval
    with torch.no_grad():
        top1avg, save_data_list = eval_model(model, criterion, test_loader, num_classes,epoch, device)
    epoch_data["test"][str(pruning_amount)].append(top1avg)
    epoch_data["train"][str(pruning_amount)].append(eval_meters.top1.avg)
    file1.write(f"Epoch {epoch} complete. Train  accuracy {str(eval_meters.top1.avg)}\n")
    file1.flush()
